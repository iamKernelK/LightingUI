...
function NotificationSystem:Notify(Options)
    ...
    for _, notif in ipairs(self.ActiveNotifications) do
        ...
        TweenService:Create(notif.Instance, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
            Position = newPos
        }):Play()
    end
    ...
    Utility:Create("UICorner", {CornerRadius = UDim.new(0, 10), Parent = notifFrame})
    ...
    if icon and icon ~= "" then
        ...
        Utility:Create("UICorner", {CornerRadius = UDim.new(0, 8), Parent = iconImg})
    end

    -- Title
    local titleLabel = Utility:Create("TextLabel", {
        Size = UDim2.new(1, icon and -70 or -24, 0, 22),
    ...
    local descLabel = Utility:Create("TextLabel", {
        Size = UDim2.new(1, icon and -70 or -24, 0, 40),
...
function AxisUI:CreateWindow(Options)
    local Window = {
        ...
        TopBarButtons = {},
        ActiveTab = nil,
        Transparency = Options.Transparency or 0.3,
    ...
    function Window:AddTopBarButton(BtnOptions)
        ...
        local btnObj = {Instance = btn, Name = name}
        function btnObj:Remove()
            btn:Destroy()
        end
        function btnObj:SetIcon(newIcon)
            btn.Image = newIcon
        end
        return btnObj
    end

    function Window:RemoveTopBarButton(name)
        for _, child in ipairs(TopBarButtonsContainer:GetChildren()) do
            if child:IsA("ImageButton") and child.Name == name then
                TweenService:Create(child, TweenInfo.new(0.2), {Size = UDim2.new(0, 0, 0, 0)}):Play()
                task.wait(0.2)
                child:Destroy()
                return true
            end
    ...
    -- Default TopBar Buttons (Order: Remove, Close, Bright)
    ...
    FloatingBtn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(MainFrame, TweenInfo.new(0.6, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = Window.Size}):Play()
    end)
    ...
    -- Opening Animation
    TweenService:Create(MainFrame, TweenInfo.new(0.8, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = Window.Size}):Play()
    ...
    function Window:CreateTab(TabOptions)
        ...
        local function GetParent(customParent)
            return customParent or TabContainer
        end
        ...
        function Tab:CreateButton(Options, parentTarget)
            ...
            return {
                SetText = function(self, newText)
                    BtnFrame:FindFirstChildOfClass("TextLabel").Text = newText
                end
            }
        end

-- 4. TextBox (تصميم احترافي مطور: النص يسار ومربع الإدخال يمين مع حدود واضحة دائماً)
        function Tab:CreateTextBox(Options, parentTarget)
            local target = GetParent(parentTarget)
            local Name = Options.Name or "Example TextBox"
            local Placeholder = Options.Placeholder or "write here"
            local Default = Options.Default or ""
            local Callback = Options.Callback or function() end
            local ClearOnFocus = Options.ClearOnFocus or false

            -- 1. الإطار الرئيسي (أسود مع تأثير زجاجي - Glassmorphism)
            ...
            -- 2. النص على اليسار (تم تكبير الحجم إلى 15 ليكون أوضح وأفخم)
            Utility:Create("TextLabel", {
                ...
                TextColor3 = Theme.TextColor,
                Font = Enum.Font.GothamMedium,
                TextSize = 15, -- حجم احترافي وواضح
                TextXAlignment = Enum.TextXAlignment.Left,
                Parent = BoxFrame
            })

            -- 3. مربع الكتابة (Input) على اليمين
            local InputBox = Utility:Create("TextBox", {
                Size = UDim2.new(0, 110, 0, 26),
                Position = UDim2.new(1, -12, 0.5, 0),
                AnchorPoint = Vector2.new(1, 0.5),
                BackgroundColor3 = Color3.fromRGB(15, 12, 8), -- أسود داكن متناسق مع ثيم الزجاج
                BackgroundTransparency = 0.1,
                Text = Default,
            ...
            -- إضافة الـ UIStroke على الـ Border وجعله ظاهراً دائماً بوضوح (Transparency = 0)
            local InputStroke = Utility:Create("UIStroke", {
                Color = Theme.StrokeColor, 
                Thickness = 1.2,
                Transparency = 0, -- ظاهر دائماً وغير شفاف
                ApplyStrokeMode = Enum.ApplyStrokeMode.Border, -- محاذي للحدود تماماً
                Parent = InputBox
            })

            -- 4. التفاعلات الحركية الناعمة عند التركيز (Focus)
            InputBox.Focused:Connect(function()
                TweenService:Create(InputStroke, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {Color = Theme.OrangePrimary}):Play()
                TweenService:Create(MainStroke, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {Color = Theme.OrangeDark, Transparency = 0}):Play()
            end)

            InputBox.FocusLost:Connect(function()
                TweenService:Create(InputStroke, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {Color = Theme.StrokeColor}):Play()
                TweenService:Create(MainStroke, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {Color = Theme.StrokeColor, Transparency = 0.3}):Play()
                Callback(InputBox.Text)
            end)

            -- 5. تأثير الهوفر (Hover) عند تمرير الماوس فوق الإطار الكامل
            BoxFrame.MouseEnter:Connect(function()
                TweenService:Create(BoxFrame, TweenInfo.new(0.2), {BackgroundColor3 = Theme.HoverElement}):Play()
            end)
            BoxFrame.MouseLeave:Connect(function()
                TweenService:Create(BoxFrame, TweenInfo.new(0.2), {BackgroundColor3 = Theme.ElementBackground}):Play()
            end)

            return {
                SetText = function(self, text) InputBox.Text = text end,
                GetText = function(self) return InputBox.Text end,
                Focus = function(self) InputBox:CaptureFocus() end
            }
        end
        ...
        function Tab:CreateToggle(Options, parentTarget)
            ...
            local function UpdateVisuals()
                ...
                if State then 
                    Utility:ApplyGradient(ToggleBg) 
                else 
        ...
        function Tab:CreateSlider(Options, parentTarget)
            ...
            return {
                ...
                GetValue = function(self)
                    return tonumber(ValBox.Text) or Default
                end
            }
        end
        ...
        function Tab:CreateKeybind(Options, parentTarget)
            ...
            Utility:Create("UIStroke", {Color = Theme.StrokeColor, Parent = KeyFrame})
            ...
            Utility:Create("UIStroke", {Color = Theme.OrangePrimary, Parent = KeyDisplay})
            ...
            KeyFrame.MouseButton1Click:Connect(function()
                ...
                Listening = true
                KeyDisplay.Text = "..."
                TweenService:Create(KeyDisplay, TweenInfo.new(0.2), {BackgroundColor3 = Theme.OrangePrimary, TextColor3 = Color3.new(1,1,1)}):Play()
                ...
                connection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
                    ...
                    if input.UserInputType == Enum.UserInputType.Keyboard then
                        ...
                        TweenService:Create(KeyDisplay, TweenInfo.new(0.2), {BackgroundColor3 = Theme.SecondaryBackground, TextColor3 = Theme.OrangePrimary}):Play()
                        Listening = false
                        connection:Disconnect()
                        Callback(CurrentKey)
                    elseif input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseButton2 then
                        KeyDisplay.Text = CurrentKey ~= Enum.KeyCode.Unknown and CurrentKey.Name or "None"
                        TweenService:Create(KeyDisplay, TweenInfo.new(0.2), {BackgroundColor3 = Theme.SecondaryBackground, TextColor3 = Theme.OrangePrimary}):Play()
                        Listening = false
                        connection:Disconnect()
                    end
        ...
        -- 9. Advanced Dropdown (Speed Hub Style)
        function Tab:CreateDropdown(Options, parentTarget)
                        ...
                        ibtn.MouseButton1Click:Connect(function()
                            ...
                            if idx then
                                ...
                                if MaxSelect == 1 then
                                    ...
                                    Arrow.Image = Theme.DropdownClosedIcon
            ...
            DropBtn.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    ...
                    Arrow.Image = Theme.DropdownOpenIcon
                    RefreshList()
            ...
            return {
                ...
                GetSelected = function(self)
                    return MaxSelect == 1 and SelectedItems[1] or SelectedItems
                end
            }
        end
